...
class MeanStream(nn.Module):
    """
    Maintains a running mean of input tensors.

    Accumulates a weighted running average during training. The running mean is stored as a
    buffer that can persist in checkpoints.

    Parameters
    ----------
    shape : Sequence[int]
        Shape of the input tensor excluding batch dimension, e.g., (C, H, W) for 3D tensors
        or (C, D, H, W) for 4D tensors.
    cap : float, optional
        Maximum effective sample count. After `cap` samples, each new batch contributes at least
        `batch_size / cap` weight to the running mean. Puts a floor on how much influence new
        samples have. Default is 100.

    Attributes
    ----------
    mean : torch.Tensor
        Running mean buffer with shape `shape`.
    count : torch.Tensor
        Scalar buffer tracking total samples seen.

    Examples
    --------
    >>> stream = MeanStream(shape=(2, 64, 64), cap=100)
    >>> x = torch.randn(4, 2, 64, 64)  # batch of 4
    >>> output = stream(x)
    >>> output.shape
    ...
    def __init__(self, shape: Sequence[int], cap: Union[float, int] = 100.0):
        super().__init__()
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        """
        Update running mean and return current estimate.

        Parameters
        ----------
        input_tensor : torch.Tensor
            Input tensor with shape (B, *shape), where B is batch size.

        Returns
        -------
        torch.Tensor
            Running mean broadcasted to input batch size, shape (B, *shape).
            During warmup (count < cap), output is scaled by count/cap.
        """
        batch_size = input_tensor.shape[0]

        if self.training:
            assert input_tensor.shape[1:] == self.shape, (
                f"Expected shape {self.shape}, got {tuple(input_tensor.shape[1:])}"
            )

            # Weight for new batch (capped to prevent vanishing contribution)
            new_count = self.count + batch_size
            alpha = batch_size / new_count.clamp(max=self.cap)
            scale = (new_count / self.cap).clamp(max=1.0)

            batch_mean = input_tensor.mean(dim=0)
            new_mean = self.mean * (1 - alpha) + batch_mean * alpha

            self.mean.copy_(new_mean)
            self.count.copy_(new_count)

            output = scale * new_mean
        else:
            scale = (self.count / self.cap).clamp(max=1.0)
            output = scale * self.mean

        return output.unsqueeze(0).expand(batch_size, *self.shape)
